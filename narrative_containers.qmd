...
![](media/chapter_containers.jpg){.rounded fig-alt="Decorative chapter image showing forested hillside in the autumn."}
...
Below are the slides that are used in the lecture component of this topic. You can cycle through them using the cursor arrows or open the [online version](https://dyerlabteaching.github.io/Data-Containers/slides.html#/title-slide){.external target="_blank"} in a new browser window.

```{=html}
<iframe src="https://dyerlabteaching.github.io/Data-Containers/slides.html#/title-slide" width="100%" height="400px"></iframe>
```
:::

::: {.content-visible unless-format="html"}
> **Lecture Slides:** Available interactively in the [online version](https://dyerlabteaching.github.io/Data-Containers/slides.html#/title-slide)

![Lecture Slides](media/slides_containers.png)
:::

## Vectors

Vectors are the most basic data container in `R`. They *must* contain data of the exact same type and are constructed using the `combine()` function, which is abbreviated as `c()` because good programmers are lazy programmers. [^narrative_containers-1]

[^narrative_containers-1]: The more lines of code that you write, the more likely there will be either a grammatical error (easier to find) or a logical one (harder to find).
...
::: {.content-visible unless-format="html"}
![Beetle Collection Locations by Stratum - Interactive map available in online version](media/map_containers_leaflet.png)
:::
